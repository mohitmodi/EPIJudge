...
public class IsStringDecomposableIntoWords {
  ...
  public static void decomposeIntoDictionaryWordsWrapper(TimedExecutor executor,
    ...
    if (!decomposable) {
      if (!result.isEmpty()) {
        throw new epi_solutions.test_framework.TestFailure("domain is not decomposable");
      }
      return;
    }

    if (result.stream().anyMatch(s -> !dictionary.contains(s))) {
      throw new epi_solutions.test_framework.TestFailure("Result uses words not in dictionary");
    }
