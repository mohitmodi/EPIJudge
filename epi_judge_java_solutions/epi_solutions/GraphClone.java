...
public class GraphClone {
  ...
  private static void checkGraph(GraphVertex node, List<GraphVertex> graph)
      throws epi_solutions.test_framework.TestFailure {
    if (node == null) {
      throw new epi_solutions.test_framework.TestFailure("Graph was not copied");
    }
    ...
    while (!q.isEmpty()) {
      ...
      if (vertex.label >= graph.size()) {
        throw new epi_solutions.test_framework.TestFailure("Invalid vertex label");
      }
      ...
      if (!label1.equals(label2)) {
        throw new epi_solutions.test_framework.TestFailure("Edges mismatch");
      }
